...
import static query.Util.docLenMap;
import static query.Util.getTF;
import static query.Util.getTTF;
...
import query.algorithm.Algo_JMSmooth;
import query.algorithm.Algorithm;
import query.readquery.Query;

/**
 * Getting score by JMsmooth
 */
public class GetScoreJM {
  ...
  public GetScoreJM(Query aQuery, Algorithm algo) {
    this.algorithm = algo;
  ...
  public static void main(String[] s) throws IOException {
    ...
    //Query query = new Query(58, "alleg corrupt offici");
    Query query = new Query(58, "alleg corrupt jurisdict government public taken");
    System.out.println(GetScoreLaplace.run(query, restClient, Algorithm.JMSmooth , 100));
    restClient.close();
  }
  ...
  private void initTTFTable(RestClient restClient) throws IOException {
    for (String word : query.getStr().split(" ")) {
      if (word.length() != 0) {
        double ttf;
        if (!ttfMap.containsKey(word)) {
          ttf = getTTF(word, restClient);
          ttfMap.put(word, ttf);
        }
      }
    }
    System.out.println(ttfMap.toString());
  }
  ...
  private void scoreEveryDocument(String word, RestClient restClient) throws IOException {
    ...
    for (String docno : scoreMap.keySet()) {
      ...
      if (halfScoreMap.containsKey(docno)) {
        ...
        //zero term frequency
        score = Algo_JMSmooth.calculate(0, scoreMap.get(docno).getLen(), ttfMap.get(word));
      }
  ...
  private Map<String, Double> scoreTerms(String word, RestClient restClient) throws IOException {
    ...
    for (JsonElement i : listOfDoc) {
      ...
      if (scoreMap.containsKey(key)) {
        double score = Algo_JMSmooth.calculate(tf, scoreMap.get(key).getLen(), ttfMap.get(word));
